...
namespace qdt
{
	...
	int QuadTreeNode::maxNumLevels = 6;

	float QuadTreeNode::m_oversizeMultiplier = 1.0f;//TODO: dunno what the fuck the benefit of this is
	...
	inline QuadTreeNode* QuadTreeNode::getChild(const sf::Vector2i &position)
	{
		return &(*m_children)[position.x][position.y];
	}

	void QuadTreeNode::getPossibleOccupantPosition(QuadTreeOccupant* pOc, sf::Vector2i &point)
	{
		...
		const sf::Vector2f &occupantCenter(pOc->m_aabb.getCenter());
		const sf::Vector2f &nodeRegionCenter(m_region.getCenter());
	...
	void QuadTreeNode::partitionTreeNode()
	{
		assert(!m_hasChildren);

		const sf::Vector2f &halfRegionDims(m_region.getHalfDims());
		const sf::Vector2f &regionLowerBound(m_region.getLowerBound());
		const sf::Vector2f &regionCenter(m_region.getCenter());
	...
	void QuadTreeNode::mergeTreeNode()
	{
		if(m_hasChildren)
		{
	...
	void QuadTreeNode::removeFromTreeNode(QuadTreeOccupant* pOc)
	{
